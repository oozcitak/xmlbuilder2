...
export abstract class BaseWriter<T extends BaseWriterOptions, U extends XMLSerializedValue> {
  ...
  private _serializeText(node: CharacterData, requireWellFormed: boolean): void {
    ...
    const markup = node.data
      .replace(nonEntityAmpersandRegex, '&amp;')
      .replace(encodeTextValueRegex, (tag) => encodeTextValueLookup[tag])
  ...
  */
  ...
  private _serializeAttributeValue(value: string | null, requireWellFormed: boolean): string {
    ...
    return value
      .replace(nonEntityAmpersandRegex, '&amp;')
      .replace(encodeAttributeValueRegex, (tag) => encodeAttributeValueLookup[tag])
  }
