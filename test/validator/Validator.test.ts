...
describe('Validator', () => {
  ...
  test('overrides', () => {
    const validate = new ValidatorImpl("1.0", {
      pubID: (val: string) => 'p' + val + 'p',
      sysID: (val: string) => 's' + val + 's',
      name: (val: string) => '_' + val + '_',
      text: (val: string) => '#' + val + '#',
      ...
      namespace: (val: string) => '@@' + val + '@@'
    })

    expect(validate.pubID('val')).toBe('pvalp')
    expect(validate.sysID('val')).toBe('svals')
    expect(validate.name('val')).toBe('_val_')
    ...
    expect(validate.namespace('val')).toBe('@@val@@')
  })
